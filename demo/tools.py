...
@tool
def scrape_website(company_name: str) -> str:
    """Scrape the content of a website and search for related information."""
    return search_and_scrape(company_name)

@tool  
def save(data: str, filename: str = "leads_output.txt") -> str:
    """Saves structured data to a text file."""
    return save_to_txt(data, filename)

# Export tools with expected names
search_tool = search
scrape_tool = scrape_website
save_tool = save